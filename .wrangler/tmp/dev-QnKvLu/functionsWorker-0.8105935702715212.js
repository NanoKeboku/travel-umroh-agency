...
// .wrangler/tmp/pages-1Fw4FK/functionsWorker-0.8105935702715212.mjs
...
__name(jsonOr, "jsonOr");
__name2(jsonOr, "jsonOr");
function rowToPaket(row) {
  return {
    id: row.id,
    jenis: row.jenis,
    ...
    durasi: row.durasi,
    durasiList: jsonOr(row.durasi_list, []),
    jadwal: row.jadwal,
    maskapai: row.maskapai,
    hotel: row.hotel,
    bandara: jsonOr(row.bandara, []),
    fasilitas: jsonOr(row.fasilitas, []),
    persyaratan: jsonOr(row.persyaratan, []),
    itinerary: jsonOr(row.itinerary, []),
    penerbangan: jsonOr(row.penerbangan, null),
    sk: jsonOr(row.sk, []),
    brosurUrl: row.brosur_url,
    gambar: row.gambar
  };
}
__name(rowToPaket, "rowToPaket");
__name2(rowToPaket, "rowToPaket");
...
var onRequestGet = /* @__PURE__ */ __name2(async ({ env, request }) => {
  ...
  const { results } = await env.ebitour_db.prepare(query).bind(...binds, limit).all();
  return json(results.map(rowToPaket));
}, "onRequestGet");
...
var onRequestGet2 = /* @__PURE__ */ __name2(async ({ env, params }) => {
  ...
  const paketRow = await env.ebitour_db.prepare("SELECT * FROM paket WHERE id = ?").bind(slug).first();
  if (!paketRow) return json2({ error: "Paket tidak ditemukan" }, 404);
  const jadwal = await env.ebitour_db.prepare(
    "SELECT tanggal, sisa_kuota, harga_quad, harga_triple, harga_double FROM jadwal WHERE paket_id = ? ORDER BY tanggal"
  ).bind(slug).all();
  const tambahan = await env.ebitour_db.prepare("SELECT label, harga FROM tambahan_pesawat WHERE paket_id = ? ORDER BY harga").bind(slug).all();
  const paket = rowToPaket(paketRow);
  paket.keberangkatan = jadwal.results.map((r) => ({
    tanggal: r.tanggal,
  ...
  paket.tambahanPesawat = tambahan.results.map((r) => ({
    label: r.label,
    harga: r.harga
  }));
  return json2(paket);
...
var onRequestGet3 = /* @__PURE__ */ __name2(async ({ env, request }) => {
  ...
  if (!paketId) {
    const { results: results2 } = await env.ebitour_db.prepare(
    ...
    return json3(results2);
  }
  const { results } = await env.ebitour_db.prepare(
    "SELECT tanggal, sisa_kuota, harga_quad, harga_triple, harga_double FROM jadwal WHERE paket_id = ? ORDER BY tanggal"
  ).bind(paketId).all();
  return json3(results);
}, "onRequestGet");
var json4 = /* @__PURE__ */ __name2((data, status = 200) => new Response(JSON.stringify(data), {
  status,
  headers: { "Content-Type": "application/json; charset=utf-8" }
}), "json");
...
// .wrangler/tmp/bundle-lt0oZ0/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lt0oZ0/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8105935702715212.js.map
